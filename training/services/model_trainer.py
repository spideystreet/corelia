...
class ModelTrainer:
    ...
    def prepare_training_args(self) -> TrainingArguments:
        ...
        args = TrainingArguments(
            ...
            num_train_epochs=get_env_int("NUM_EPOCHS") if os.getenv("NUM_EPOCHS") else training_config["num_train_epochs"],
            per_device_train_batch_size=get_env_int("BATCH_SIZE") if os.getenv("BATCH_SIZE") else training_config["per_device_train_batch_size"],
            per_device_eval_batch_size=get_env_int("BATCH_SIZE") if os.getenv("BATCH_SIZE") else training_config["per_device_eval_batch_size"],
            gradient_accumulation_steps=get_env_int("GRADIENT_ACCUMULATION_STEPS") if os.getenv("GRADIENT_ACCUMULATION_STEPS") else training_config["gradient_accumulation_steps"],
            learning_rate=get_env_float("LEARNING_RATE") if os.getenv("LEARNING_RATE") else training_config["learning_rate"],
            weight_decay=training_config["weight_decay"],
            warmup_steps=get_env_int("WARMUP_STEPS") if os.getenv("WARMUP_STEPS") else training_config["warmup_steps"],
            logging_steps=get_env_int("LOGGING_STEPS") if os.getenv("LOGGING_STEPS") else training_config["logging_steps"],
            save_steps=get_env_int("SAVE_STEPS") if os.getenv("SAVE_STEPS") else training_config["save_steps"],
            ...
            fp16=get_env_bool("FP16") if os.getenv("FP16") else training_config["fp16"],
